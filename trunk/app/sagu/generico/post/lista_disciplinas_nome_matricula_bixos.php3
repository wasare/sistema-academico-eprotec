...
function _select(ofer,id,nome,prof,dia,ref_curso,turno,status)
{
    window.opener.setResult(ofer,id,nome,prof,dia,ref_curso,turno,status);
    window.close();
}
...
<form method="post" name="myform" action="lista_disciplinas_nome_matricula_bixos.php3">
...
if ( $id != '' || $desc != '' )
{
  ...
  for ( $i=0; $query->MoveNext(); $i++ )
  {
    ...
    $href1 = "<a href=\"javascript:pre_requisitos('/generico/mostra_pre_requisitos.phtml?ref_disciplina=$id&ref_curso=$curso')\"><img src=\"../images/info.gif\" border=0 alt='Ver Pré Requisitos'></a>";
    ...
    if ($count > 1)
    {
        $href = "<a href=\"javascript:_select($ofer,$id,'$nome','$prof','$idia','$ref_curso','$iturno','$status')\"><img src=\"../images/select.gif\" border=0></a>";
    }
    else
    {
       if ($status == 1)
       ...
       ?>
       <script language="JavaScript">
           window.opener.setResult(<? echo("'$ofer','$id','$nome','$prof','$idia','$ref_curso','$iturno','$status'") ?>);
           window.close();
       </script>
       <?
       }
    ...
    <tr bgcolor="<?echo($bg);?>" valign="top">
      ...
      <td width="7%" align="center"> <font face="Arial, Helvetica, sans-serif" size="2"><?echo($dia);?></font></td>
