...
function Mostra_Matricula($id_contrato, $ref_curso, $ref_campus, $ref_periodo, $aluno_id, $conn )
{
  ...
  SaguAssert($mes && $mes_resto, "Preencha o mês de seqüência e o numero de parcelas!!!");
  flush();
  ...
  if ($query->MoveNext())
  {
    ...
    if (!$ultimo_mes)
    {
        SaguAssert(0, "Aluno sem financeiro (previsões) no <br>contrato <b>$ref_contrato</b> no período <b>$periodo_id</b>.");
    }
  }
  ...
  /********************  GRAVA INFORMAÇÕES NO FINANCEIRO **********************/
