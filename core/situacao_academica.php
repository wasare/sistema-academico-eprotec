    ...
    if ($diarios_matriculados > 0) {
		...
		WHERE
                    ...
                    ( m.num_faltas > ((
                        ...
                        WHERE ref_disciplina_ofer = $diario_id ) * 0.25
                    )) AND
                    o.id = $diario_id; ";
...
function verificaAprovacao($aluno_id, $curso_id, $diario_id) {
    ...
    $sqlDisciplina = "
        SELECT DISTINCT o.id AS diario
        FROM
...
function verificaEquivalencia($curso_id, $diario_id) {
    ...
    $sqlDisciplina = "
        SELECT DISTINCT
         ...
         WHERE 
            ref_disciplina = get_disciplina_de_disciplina_of('$diario_id') AND
            ref_curso = '$curso_id';";
...
function verificaPeriodo($periodo_id) {
    ...
    $sqlPeriodo = "SELECT dt_final FROM periodos WHERE id = '$periodo_id';";
